...
class EncodedNumber(object):
    ...
    def __init__(self, n, max_int, encoding, exponent):
        self.n = n
        ...
        *max_exponent*, and such that the number *precision* is not
        ...
        if precision is None:
            if isinstance(scalar, int):
                ...
                # 浮点数的尾数部分长度为FLOAT_MANTISSA_BITS，有效位数为FLOAT_MANTISSA_BITS，所以乘以bin_lsb_exponent可以保证计算上的准确性，此时的位数为以2为底
                bin_lsb_exponent = bin_flt_exponent - cls.FLOAT_MANTISSA_BITS

                # What's the corresponding base BASE exponent? Round that down.
                # 需要将以2为底的切换为以BASE为底，用到了换底公式：log_a b = log_c b / log_c a
                # 将bin_lsb_exponent换为以BASE为底。
            ...
            # print(f"prec_exponent = {prec_exponent}")
        ...
        # print(-exponent)
        # 手动使用定点表示float，将float的尾数和指数部分结合成一个数，此时使用fractions库的原因是该库可以得到准确的结果。
        ...
        return cls(n, max_int, int_rep % n, exponent)
    ...
    def decrease_exponent_to(self, new_exp):
        ...
        return self.__class__(self.n, self.max_int, new_enc, new_exp)
...
class EncodedVector(object):
    ...
    def __init__(self, n, max_int, encoding:np.ndarray, exponent):
        self.n = n
        self.max_int = max_int
        self.encoding = encoding
        self.exponent = exponent
    ...
    @classmethod
    def encode(cls, n, max_int, vector, precision=None, max_exponent=None):
        # Calculate the maximum exponent for desired precision
        if precision is None:
            if isinstance(vector[0], int) or isinstance(vector[0], np.int64):
                prec_exponent = 0
            elif isinstance(vector[0], float):
                # Encode with *at least* as much precision as the python float
                # What's the base-2 exponent on the float?
                bin_flt_exponent = math.frexp(vector[0])[1]
        ...
        # Use rationals instead of floats to avoid overflow.
        int_rep = np.array([round(fractions.Fraction(i)
                        * fractions.Fraction(cls.BASE) ** -exponent) for i in vector])

        if abs(int_rep[0]) > max_int:
            raise ValueError('Integer needs to be within +/- %d but got %d'
                             % (max_int, int_rep[0]))
    ...
    def decode(self):
        ...
        for encode in self.encoding:
            if encode >= self.n:
                # Should be mod n
                raise ValueError('Attempted to decode corrupted number')
            elif encode <= self.max_int:
                # Positive
                mantissa = encode
            elif encode >= self.n - self.max_int:
                # Negative
                mantissa = encode - self.n
            else:
                raise OverflowError('Overflow detected in decrypted number')
            decodelist = np.append(decodelist, mantissa)
        # decodelist = np.array(decodelist)
        # decodelist.astype(float)
        # print(f"type of decodelist = {type(decodelist)}")
        if self.exponent >= 0:
            # Integer multiplication. This is exact.
            return decodelist * self.BASE ** self.exponent
        else:
            # BASE ** -e is an integer, so below is a division of ints.
            # Not coercing mantissa to float prevents some overflows.
            try:
